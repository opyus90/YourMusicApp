...
var artist = JSON.parse( $(".artist").attr("data-artist") );




function getArtist(){
	var songs = artist[0]["songs"].split(", ");
	$(".artist_image").replaceWith('<div class="artist_image text-right"><img src="images/artist/'+artist[0]["name"]+'_front.jpg" alt="">'
	                              +'<img src="images/artist/'+artist[0]["name"]+'_back.jpg" alt=""></div>');
	$(".artist_info_list").replaceWith('<div class="artist_info_list"><ul><li>Born: <span>'+artist[0]["borns"]+'</span></li>'
	                              +'<li>First release: <span>'+artist[0]["firstrelease"]+'</span></li>'
								  +'<li>First album: <span>'+artist[0]["firstalbum"]+'</span></li></ul></div>');
	$(".songs_title").replaceWith('<div class="songs_title"><h1>'+artist[0]["albums"]+'</h1></div>');
...
$(document).ready(function() {
	...
	$(".search-title").on('keypress',function(e) {
        if( (e.which == 13) && ($('.search-title').val().length > 0) ) {
		   ...
		   e.preventDefault();
		   $('.search-title').val('');  
		   window.location.href = "/artists_search_"+val;
        }
    });
